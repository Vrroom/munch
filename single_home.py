...
from infinigen.assets.materials import ice, bark_birch, dirt, lava, mud, sand, sandstone, cobble_stone, water, snow
...
import utils, drawTools, node, scope, production, graphTools, bbox, roofTools, assetDict, cameraTools, fire_sim
reload(utils)
reload(drawTools)
reload(node)
reload(scope)
...
def generate_house (origin=np.array([0,0,0])) : 
    #####################################################################
    ## PERLIN NOISE
    #####################################################################
    ...
    FOOTPRINT = Scope(3, origin, np.eye(3), [FLOOR_WIDTH, FLOOR_LENGTH, GROUND_BASE + (NUM_FLOORS + 1) * FLOOR_HEIGHT])
    ...
    roof = Production(
        ...
        succ=[
            ['roof_literal', 'chimney_literal', 'epsilon'],
            ['roof_literal', 'chimney_literal', 'smoke_literal'],
        ],
        prob=[0.5, 0.5]
    )
    ...
    for node, obj in zip (leaf_nodes, all_objs) : 
        if node.id == 'smoke_literal': 
            make_fire_in_scope(node.scope, obj)
